...
class TechnologyHomeController extends GetxController {
  TechnologyHomeController(this.homeCreatorModelObj);
  ...
  Rx<TechnologyHomeModel> homeCreatorModelObj;
  ...
  void getInfluencers() async {
    try {
      ...
      trendingInfluencers = allInfluencers
          .where((influencer) => influencer.niche!.contains(["Tech & Gaming" , "Science & Technology"]))
          .toList();
      if (trendingInfluencers.isEmpty) {
        error('No influencers found for the technology niche');
      } else {
        error('');
  ...
  void getRecommended() async {
    ...
    try {
      ...
      if (recommendedInfluencers.isEmpty) {
        error('No recommended influencers found for the technology niche');
      } else {
        error('');
